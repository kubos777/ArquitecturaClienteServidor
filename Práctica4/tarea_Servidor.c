...
int main(){
	...
	if( read( cliente_socket_fd, &longitud, sizeof(longitud) ) == 0 ){
		printf("El cliente  ha cerrado la sesión\n");
		return 0;
	}
	...
	printf("El mensaje del cliente es : %s ", mensaje);

 	printf("Responder al cliente: ");
 	const char* const mensajepCliente = fgets(respuesta, 256, stdin);
 	int tamMensaje = strlen( mensajepCliente ) + 1;
 	write ( cliente_socket_fd, &tamMensaje, sizeof(tamMensaje) );
 	write ( cliente_socket_fd, mensajepCliente, tamMensaje );
